...
def insert_max_category_in(max_category, start_row, worksheet):
    ...
    worksheet.insert_row(["Category with Max Expense", max_category], start_row+6)
    time.sleep(2)

def insert_biggestTs_in(start_row, worksheet):
    # Insert the header for the biggest transactions section
    worksheet.insert_row(["Biggest Transaction in Each Month", "", "", "", ""], start_row + 7)
    ...
    for (year, month), data in biggestTs.items():
        if data['transaction']:
            ...
            worksheet.insert_row([f"{year}-{month:02d}", transaction_type, amount, description, category], start_row + 8)
            time.sleep(2)
